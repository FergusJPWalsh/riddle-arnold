...
The file is in `.tsv` format: a `TAB` seperates the headword from its entry.
...
The files ```riddle-arnold.tsv``` and ```riddle-arnold-proper-names.tsv``` contain the dictionary entries. They can be opened in a plain text editor, internet browser or Excel.
